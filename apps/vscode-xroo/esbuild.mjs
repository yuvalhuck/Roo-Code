...
async function main() {
	const name = "extension-xroo"
	const production = process.argv.includes("--production")
	const minify = production
	const sourcemap = !production

	const overrideJson = JSON.parse(fs.readFileSync(path.join(__dirname, "package.xroo.json"), "utf8"))
	console.log(`[${name}] name: ${overrideJson.name}`)
	console.log(`[${name}] version: ${overrideJson.version}`)
	console.log(`[${name}] publisher: ${overrideJson.publisher}`)
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": `"${overrideJson.name}"`,
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"XRoo"',
			...
			setup(build) {
				build.onEnd(() => {
					...
					const generatedPackageJson = generatePackageJson({
						packageJson,
						overrideJson,
						substitution: ["roo-cline", overrideJson.name],
					})
					...
					const nlsXrooPkg = JSON.parse(
						fs.readFileSync(path.join(__dirname, "package.nls.xroo.json"), "utf8"),
					)

					fs.writeFileSync(
						path.join(buildDir, "package.nls.json"),
						JSON.stringify({ ...nlsPkg, ...nlsXrooPkg }, null, 2),
					)
